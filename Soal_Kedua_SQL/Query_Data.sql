...
-- Menambahkan data ke tabel kurs
...
CREATE TABLE karyawan (
    ...
    CONSTRAINT karyawan_valuta_fk_kurs FOREIGN KEY (valuta) REFERENCES kurs(valuta)
);
...
SELECT 
    ...
    END range_umur, COUNT (karyawan.nip) ||' orang' AS total_karyawan, SUM (karyawan.gaji * kurs.kurs) AS total_gaji
FROM karyawan
...
SELECT * FROM (
	SELECT karyawan.valuta, COUNT(karyawan.nip) AS total_karyawan,
		SUM(karyawan.gaji * kurs.kurs) AS total_gaji
	FROM karyawan
	LEFT JOIN kurs ON kurs.valuta = karyawan.valuta
	GROUP BY 1
) total
WHERE total.total_gaji > 15000000;
